...
// Since Yomi is operating without a database, it uses the RAM of the Node instance.
...
const MAX_CACHE_ENTRIES = 2000; // Hard limit to prevent memory leaks from unique queries
...
async function searchSukebeiForHentai(romajiTitle) {
    ...
    const fetchPromise = (async () => {
        ...
        try {
            ...
            // SAFETY NET: Prevent memory leaks if too many unique search queries pile up
    ...
    // SAFETY NET: Running promises also take up RAM, clear if threshold is reached
    if (searchCache.size > MAX_CACHE_ENTRIES) {
        searchCache.clear();
    }
