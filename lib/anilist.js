...
async function searchAdultAnime(query) {
    // Added description, coverImage, bannerImage to search query
...
async function getTrendingAdultAnime() {
    // Added description, coverImage, bannerImage to trending query
    const q = `query ($sort: [MediaSort]) { 
...
async function getTopAdultAnime() {
    // Added description, coverImage, bannerImage to top rated query
    const q = `query ($sort: [MediaSort]) { 
        Page(page: 1, perPage: 30) { 
            media(type: ANIME, isAdult: true, sort: $sort) { 
                id 
...
async function getAnimeMeta(anilistId, retries = 3) {
    ...
    const q = `query ($id: Int) { Media(id: $id, type: ANIME) { id idMal title { romaji english } synonyms coverImage { extraLarge } bannerImage description format episodes nextAiringEpisode { episode airingAt } streamingEpisodes { title thumbnail } averageScore status seasonYear startDate { year month day } } }`;
    const fetchPromise = (async () => {
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
