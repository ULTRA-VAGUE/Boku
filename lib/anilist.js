...
// Einmal abgerufene Daten bleiben für 6 Stunden im Speicher.
// ============================================================================
...
async function _fetchAniList(query, variables, retries = 3) {
    // 1. Cache-Prüfung: Haben wir diese exakte Abfrage in den letzten 6 Stunden schon gemacht?
    ...
    // 2. Fetch mit Retry-Schleife für den Fall von Server- oder Rate-Limit-Fehlern
    for (let attempt = 0; attempt < retries; attempt++) {
        try {
            ...
            // Speichern im Cache (max. 2000 Einträge, um RAM-Overflow bei langen Laufzeiten zu verhindern)
            apiCache.set(cacheKey, { timestamp: Date.now(), data: results });
            if (apiCache.size > 2000) apiCache.clear();
            ...
            if (attempt === retries - 1) return []; // Letzter Versuch fehlgeschlagen, wir geben auf
...
async function getTrendingAdultAnime() {
    ...
    // Fiktive Variable "type" für einen eindeutigen String-Hash im Cache
...
async function getJikanMeta(cleanedTitle, retries = 3) {
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
        try {
            ...
            // Jikan sperrt extrem schnell. Wir warten hier deutlich länger bei einem 429.
