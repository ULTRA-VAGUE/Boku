        ...
        try {
            ...
            const results = response.data.data.Page.media.map(anime => {
                ...
                return {
                    ...
                    // Pass the newly formatted date object
...
async function searchAdultAnime(query) {
    ...
    // Added 'day' to startDate to allow precise Stremio sorting
    const graphqlQuery = `
        query ($search: String) { 
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
        try {
            ...
            const result = {
                ...
                type: 'series',
                name: cleanTitle,
                // Passing altName and synonyms explicitly for the Stream Handler Fallback Engine
    ...
    for (let attempt = 0; attempt < retries; attempt++) {
        try {
            ...
            if (data && data.length > 0) {
                ...
                // Extract precise date values from Jikan's exact broadcast object
                ...
                const result = {
                    ...
                    // Pass English title and synonyms to empower the Fallback Engine
