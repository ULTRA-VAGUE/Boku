...
// NEW: Shielded IDs using 'yomi:' prefix to prevent hostile Cinemeta metadata overwrites.
...
function formatDescription(anime) {
    ...
    // Catch malformed or double-multiplied scores (e.g. 900%) and normalize them to a 100% scale.
        ...
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                const results = response.data.data.Page.media.map(anime => {
                    ...
                    // Prefix 'yomi:' shields the catalog item from Stremio Cinemeta tracking algorithms.
                    return {
