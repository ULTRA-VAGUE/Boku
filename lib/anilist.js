...
async function getAnimeMeta(anilistId, retries = 3) {
    ...
    // DUAL-QUERY ENGINE: Attempts safe Media query first. If blocked (null), breaches the 18+ lock using the Page wrapper.
    const qMedia = `query ($id: Int) { Media(id: $id, type: ANIME) { id idMal title { romaji english } synonyms coverImage { extraLarge } bannerImage description format episodes nextAiringEpisode { episode airingAt } streamingEpisodes { title thumbnail } averageScore status seasonYear startDate { year month day } } }`;
    const qPage = `query ($id: Int) { Page(page: 1, perPage: 1) { media(id: $id, type: ANIME, isAdult: true) { id idMal title { romaji english } synonyms coverImage { extraLarge } bannerImage description format episodes nextAiringEpisode { episode airingAt } streamingEpisodes { title thumbnail } averageScore status seasonYear startDate { year month day } } } }`;
    ...
    const fetchPromise = (async () => {
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                try {
                    const res = await axios.post(ANILIST_URL, { query: qMedia, variables: { id: parseInt(anilistId) } }, { timeout: 4000 });
                    anime = res.data?.data?.Media;
                } catch(e) {}

                if (!anime) {
                    const resAdult = await axios.post(ANILIST_URL, { query: qPage, variables: { id: parseInt(anilistId) } }, { timeout: 4000 });
                    anime = resAdult.data?.data?.Page?.media?.[0];
                }
                ...
                console.error("[AniList] Meta Error (Attempt " + (attempt + 1) + "/" + retries + "):", error.message);
    ...
    const fetchPromise = (async () => {
        for (let attempt = 0; attempt < retries; attempt++) {
            try {
                ...
                console.error("[Jikan MAL] Fallback Error:", error.message);
                if (attempt < retries - 1) await sleep(1500);
            }
        }
        apiCache.delete(cacheKey);
